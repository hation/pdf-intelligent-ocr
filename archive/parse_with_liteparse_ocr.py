#!/usr/bin/env python3
"""
使用 LiteParse 并启用 OCR 功能解析所有 PDF 文件并保存为 Markdown 格式
"""
...
def parse_pdf_with_liteparse_ocr(input_dir, output_dir):
    """使用 LiteParse 并启用 OCR 解析 input_dir 中的所有 PDF 文件"""
    ensure_directory_exists(output_dir)
    
    # 创建解析器实例，启用 OCR 以提高图像识别率
    ...
    for filename in pdf_files:
        ...
        output_filename = os.path.splitext(filename)[0] + '_liteparse.md'
...
def main():
    """主函数"""
    parser = argparse.ArgumentParser(description='使用 LiteParse 并启用 OCR 解析 PDF 文件并保存为 Markdown')
    
    parser.add_argument('input_dir', nargs='?', default='files', 
                      help='输入目录（默认: files）')
    parser.add_argument('output_dir', nargs='?', default='outputs_liteparse_ocr',
                      help='输出目录（默认: outputs_liteparse_ocr）')
    ...
    parse_pdf_with_liteparse_ocr(args.input_dir, args.output_dir)
