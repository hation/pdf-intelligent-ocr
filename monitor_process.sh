#!/bin/bash
# 进程监控脚本 - 用于大规模PDF OCR处理的资源监控

INPUT_DIR="files"
OUTPUT_DIR="outputs_batch_optimized"
...
# 开始处理
start_processing() {
    if pgrep -f "batch_ocr_optimized.py" > /dev/null; then
        echo "处理进程已在运行"
        return 1
    fi
    ...
    nohup python3 batch_ocr_optimized.py "$INPUT_DIR" "$OUTPUT_DIR" -r >> "$LOG_FILE" 2>&1 &
...
stop_processing() {
    if pgrep -f "batch_ocr_optimized.py" > /dev/null; then
        echo "正在停止处理进程..."
        pkill -f "batch_ocr_optimized.py"
        sleep 2
        
        if pgrep -f "batch_ocr_optimized.py" > /dev/null; then
            pkill -9 -f "batch_ocr_optimized.py"
        fi
