...
- **Word 文档支持**: 自动转换 `.docx`（markitdown）和 `.doc`（macOS textutil）为 Markdown，与 PDF 一样纳入总结流程
...
- **文件自动移动**: 处理完成后自动归档源文件
- **非PDF自动清理**: 启动时自动将 `files/` 中的非文档文件（xlsx/pptx/图片等）移动到 `~/Downloads`；Word 文档（docx/doc）会被转换为 Markdown 纳入总结流程，转换后源文件归档到 `files_processed/`
