    ...
    try {
        ...
        const extFilter = opts.extension
            ? opts.extension.startsWith(".")
                ...
                : `.${opts.extension.toLowerCase()}`
            : undefined;
        const files = collectFiles(inputDir, !!opts.recursive, extFilter);
        if (files.length === 0) {
            console.error(`[liteparse] no matching files found in ${inputDir}`);
            return;
        }
        if (!opts.quiet) {
            console.error(`[liteparse] found ${files.length} files to process`);
        }
        let success = 0;
        let errors = 0;
        for (const filePath of files) {
            ...
            try {
                ...
                if (!opts.quiet) {
                    ...
                    console.error(`[liteparse] ${filePath} → ${outPath} (${elapsed}ms)`);
                }
            }
            catch (err) {
                console.error(`[liteparse] error parsing ${filePath}: ${err instanceof Error ? err.message : String(err)}`);
                errors++;
            }
        }
        console.error(`[liteparse] batch complete: ${success} succeeded, ${errors} failed`);
        if (errors > 0)
            process.exit(1);
    }
    catch (err) {
...
function collectFilesInner(dir, recursive, extFilter, files) {
    for (const entry of readdirSync(dir, { withFileTypes: true })) {
        ...
        if (entry.isDirectory()) {
            if (recursive)
                collectFilesInner(fullPath, recursive, extFilter, files);
            continue;
        }
