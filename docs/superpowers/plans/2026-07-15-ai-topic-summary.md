...
**Goal:** Build a reusable topic summary extractor that generates a detailed AI-focused report with one-sentence summaries and key insights, sub-categorized for easy navigation.

**Architecture:** Single Python script that reads from existing summary files, applies configurable categorization rules, and outputs a structured markdown report. The script is designed to be extensible for future topics beyond AI.

**Tech Stack:** Python 3.x, standard library only (os, re, argparse, json)
...
|------|--------------|------|
| `extract_topic_summary.py` | Main script: argument parsing, file reading, categorization, report generation | Create |
| `output/topic_summaries/AI/AI_documents_summary_20260714.md` | Output report file | Generated |

---

### Task 1: Create Script Foundation and Configuration

**Files:**
- Create: `extract_topic_summary.py`

- [ ] **Step 1: Write script skeleton with topic configuration**
...
```bash
git add extract_topic_summary.py
git commit -m "feat: add topic summary script skeleton with config"
```

---

### Task 2: Implement Document Parsing from Summary List

**Files:**
- Modify: `extract_topic_summary.py` (append)

- [ ] **Step 1: Add function to parse summary_list.md**
...
- [ ] **Step 2: Test the parsing function manually**

Run:
```python
# In Python REPL
from extract_topic_summary import parse_summary_list
docs = parse_summary_list('output/daily/20260714/reports/summary_list_20260714.md', 'AI与科技')
print(f"Found {len(docs)} AI docs")
# Expected: 40-50 docs
```

- [ ] **Step 3: Commit the parsing function**

```bash
git add extract_topic_summary.py
git commit -m "feat: add summary_list.md parsing function"
```

---

### Task 3: Implement Sub-category Classification Logic

**Files:**
- Modify: `extract_topic_summary.py` (append)

- [ ] **Step 1: Add sub-categorization function**
...
- [ ] **Step 2: Add test for categorization**
...
```bash
python3 -c "from extract_topic_summary import test_categorization; test_categorization()"
# Expected: "Categorization tests passed"
```

- [ ] **Step 4: Commit the categorization logic**

```bash
git add extract_topic_summary.py
git commit -m "feat: add sub-category classification logic"
```

---

### Task 4: Implement Summary Reader Function

**Files:**
- Modify: `extract_topic_summary.py` (append)

- [ ] **Step 1: Add function to read individual summary files**
...
- [ ] **Step 2: Test the reader with a known file**

```bash
python3 -c "
from extract_topic_summary import read_document_summary
result = read_document_summary('output/daily/20260714/summaries', '2026中国具身智能产业发展报告-89页_hybrid')
print(result['one_sentence'][:100] if result else 'Not found')
"
# Expected: Should show part of the summary
```

- [ ] **Step 3: Commit the summary reader**

```bash
git add extract_topic_summary.py
git commit -m "feat: add document summary reader function"
```

---

### Task 5: Implement Markdown Report Generation
...
- [ ] **Step 2: Commit the report generator**
...
### Task 6: Implement Main Function and CLI Interface

**Files:**
- Modify: `extract_topic_summary.py` (append)

- [ ] **Step 1: Add main function with argument parsing**
...
- [ ] **Step 2: Commit the main function**

```bash
git add extract_topic_summary.py
git commit -m "feat: add main function and CLI interface"
```

---

### Task 7: Test End-to-End Execution

**Files:**
- Generated: `output/topic_summaries/AI/AI_documents_summary_20260714.md`

- [ ] **Step 1: Run the script with default parameters**
...
**Expected Output:**
```
📄 使用汇总列表：summary_list_20260714.md
📋 找到 49 份AI与科技相关文档
✅ 成功读取 45+ 份文档总结
✅ 报告已生成：output/topic_summaries/AI/AI_documents_summary_20260714.md
   包含 45+ 份文档
```

- [ ] **Step 2: Verify output file exists and has correct structure**

```bash
head -30 output/topic_summaries/AI/AI_documents_summary_20260714.md
```
...
- [ ] **Step 3: Commit the test run and add .gitignore for output**

```bash
echo "output/topic_summaries/" >> .gitignore
git add .gitignore
git commit -m "chore: add topic_summaries to gitignore"
```
...
- [ ] **Step 1: Ensure output directories are properly ignored**

```bash
# Check if already in .gitignore
grep -q "topic_summaries" .gitignore || echo "output/topic_summaries/" >> .gitignore
```

- [ ] **Step 2: Run final end-to-end test**

```bash
python3 extract_topic_summary.py --help
# Expected: Shows help with all options

python3 extract_topic_summary.py
# Expected: Full successful run with report generated
```

- [ ] **Step 3: Final commit with all changes**

```bash
git add .gitignore extract_topic_summary.py
git commit -m "feat: complete AI topic summary extraction tool"
...
**1. Spec Coverage:**
- ✅ Topic filtering from existing "AI与科技" category → Task 2
- ✅ One-sentence summary per document → Task 4
- ✅ 5 key points per document → Task 4
- ✅ 4 sub-categories with keyword matching → Task 3
- ✅ Report structure with overview stats → Task 5
- ✅ Output location: output/topic_summaries/AI/ → Task 6
- ✅ Extensible design for future topics → TOPIC_RULES config

**2. Placeholder Scan:**
- ✅ No TBD/TODO in steps
- ✅ All code blocks contain actual working code
- ✅ Every step shows exact commands with expected output
- ✅ All file paths are specific (no "path/to/file")

**3. Type Consistency:**
- ✅ Function names are consistent (parse_summary_list, categorize_document, etc.)
- ✅ Variable names match across tasks (topic_config, docs_by_category)
- ✅ Path conventions are consistent (output/topic_summaries/<topic>)
- ✅ Task 4 uses same TOPIC_RULES structure from Task 1
